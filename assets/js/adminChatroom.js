...
function confirmKick(memberName, groupName) {
  ...
  if (!modal || !body || !yesBtn) return;

  body.textContent = `Kick ${memberName} from ${groupName}?`;

  const toRemove = Array.from(document.querySelectorAll('.member-item')).find(item => 
      item.querySelector('span')?.textContent === memberName
  );
  ...
  newYesBtn.addEventListener('click', () => {
      if (toRemove) {
          toRemove.remove();
          updateMemberCount();
          alert(`${memberName} kicked from ${groupName}`);
      }
      modal.hide();
  });
